...
export function CapacityModel() {
  ...
  return (
    <div className="model-block">
      ...
      {/* The two capacities, on one shared scale. Never two axes. */}
      <p className="ctl-label">What each side can take, on the same scale</p>
      <ul className="caps">
        <li>
      ...
      <div className="verdict">
        ...
        <div className="v-cell">
          ...
          <span>files on the desktop in 5 years</span>
        </div>
      </div>

      {/* Strain: one hue, light to dark. Never a green-amber-red rainbow. */}
      ...
      <p className="caveat">
        Their current volume, around two thousand a year, is the only operating
        point anyone gave us, and they said it already crashes there. Double was
        described as collapse. Everything above is drawn from those two
        statements, not from a measurement, and nobody has diagnosed the crashes.
        Build capacity is taken as scaling with lines, which is the optimistic
        reading. The order desk is taken as scaling with people, which is the
        generous one: training others has not stuck, so a second pair of hands
        is not really a second desk.
      </p>
    </div>
  );
}
